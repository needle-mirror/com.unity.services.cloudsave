//-----------------------------------------------------------------------------
//     This file was mostly generated by the C# SDK Code Generator. Some additions were made
//     in order to handle XML responses, as provided by GCS in the case of an error. The custom
//     blocks are indicated by comments.
//
...
namespace Unity.Services.CloudSave.Internal.Http
{
    ...
    internal static class ResponseHandler
    {
        ...
        {
            // Begin custom block
            if (typeof(T) == typeof(GCSErrorResponse))
            {
                return (T)(new XmlSerializer(typeof(T)).Deserialize(
                    new StringReader(GetDeserializedJson(response.Data))));
            }
            // End custom block
        ...
        {
            // Begin custom block
            if (type == typeof(GCSErrorResponse))
            {
                return new XmlSerializer(type).Deserialize(
                    new StringReader(GetDeserializedJson(response.Data)));
            }
            // End custom block
